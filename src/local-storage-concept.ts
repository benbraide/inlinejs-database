...
export class LocalStorageConcept implements IDatabaseConcept {
    private wrappers_: Record<string, LocalStorageWrapper> = {};

    public Wrap(name: string): LocalStorageWrapper {
        return (this.wrappers_[name] || (this.wrappers_[name] = new LocalStorageWrapper(name)));
    }
    ...
    private GetWrapper_(name: string): LocalStorageWrapper {
        return (this.wrappers_[name] || (this.wrappers_[name] = new LocalStorageWrapper(name)));
    }
}